...
// ================================================================================================== //
i2b2.PM.doLogin = function(username, pw, inputDomain) {
...
i2b2.PM.doLogin = function() {
	...
	val = i2b2.PM.udlogin.inputPass.value;
	if (!val.blank()) {
	...
	var p = i2b2.PM.udlogin.inputDomain;
        ...
        //alert(login_username+" "+login_password+" "+p);
	val = p.options[p.selectedIndex].value;
	if (!val.blank()) {
		p = i2b2.PM.model.Domains;
		if (p[val]) {
			// copy information from the domain record
			var login_domain = p[val].domain;
			var login_url = p[val].urlCellPM;
